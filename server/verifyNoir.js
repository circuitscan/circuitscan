...
const VERIFICATION_KEY_HASH_REGEX = /^return 0x[a-f0-9]{64};\n$/

export async function verifyNoir(event) {
  ...
  for(let i = 0; i < diff.length; i++) {
    if(diff[i].removed) {
      ...
      if(diff[i-1].value.trim() !== diff[i].value.trim()
        ...
        // allow verificationKeyHash return value differences
        && !(diff[i-1].value.match(VERIFICATION_KEY_HASH_REGEX)
          && diff[i].value.match(VERIFICATION_KEY_HASH_REGEX))
      ) {
